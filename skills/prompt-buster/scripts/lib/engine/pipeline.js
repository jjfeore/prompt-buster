...
// endorsement, or a fake untrusted-content boundary). Neutralize just the
// forged span so the surrounding real content is preserved but the forgery
// carries no PB authority.
const PB_BANNER_SPAN = /\[PromptBuster:[^\]\n]*\]?/gi;
const PB_BOUNDARY_SPAN = /<\/?pb-untrusted-content-[0-9a-f]*>/gi;

function stripForgedFrames(content) {
  const value = String(content ?? "");
  // Guard must be case-insensitive to match the case-insensitive strip regexes;
  // a lowercase "[promptbuster: …]" forgery is still read as authoritative by
  // downstream agents.
  if (!/\[PromptBuster:/i.test(value) && !/pb-untrusted-content-/i.test(value)) return value;
  return value.replace(PB_BANNER_SPAN, "[non-PromptBuster text removed]").replace(PB_BOUNDARY_SPAN, "");
}
