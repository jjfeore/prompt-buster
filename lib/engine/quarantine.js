...
function recordDecision(kind, contentSha256, { id, note = "", ttlHours, source = {} }) {
  const { decisions, corrupt } = readDecisions();
  // Never persist a decision on top of a corrupt in-memory set — that would
  // atomically overwrite (and permanently wipe) every previously stored deny.
  if (corrupt) {
    throw new QuarantineError(
      `refusing to write decisions over a corrupt ${decisionsPath()} — move it aside and retry (existing deny/allow records could not be read)`,
    );
  }
