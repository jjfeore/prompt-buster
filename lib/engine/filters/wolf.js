...
import { createWriteStream, existsSync, readFileSync, mkdirSync, renameSync, statSync, copyFileSync } from "node:fs";
...
async function downloadTo(url, dest) {
  ...
  const tmp = `${dest}.part`;
  await streamPipeline(Readable.fromWeb(response.body), createWriteStream(tmp));
  renameSync(tmp, dest);
}
